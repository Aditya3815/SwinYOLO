#!/usr/bin/env python3
"""
infer_dota.py — DA-YOLO Inference on DOTA 1.5 Aerial Tiles
===========================================================
Runs inference on DOTA 1.5 val images (1024×1024 pre-tiled patches)
using the best trained checkpoint.

Outputs (saved to --output, default: infer_output/dota):
    infer_output/dota/
...
Usage (from project root):
    # Default: first 20 val images, DOTA v1 checkpoint
    python infer_dota.py

    # All val images:
    python infer_dota.py --max-images -1
...
# 16 visually distinct BGR colors
_COLORS_BGR = [
    ( 44, 160, 214),   # 0  plane              — blue
    (255,  99,  71),   # 1  ship               — tomato
    ( 44, 160,  44),   # 2  storage-tank       — green
    (148, 103, 189),   # 3  baseball-diamond   — purple
    (214, 139,   0),   # 4  tennis-court       — amber
    (227, 119, 194),   # 5  basketball-court   — pink
    ( 23, 190, 207),   # 6  ground-track-field — cyan
    (188, 189,  34),   # 7  harbor             — yellow-green
    (140,  86,  75),   # 8  bridge             — brown
    (255, 127,  14),   # 9  large-vehicle      — orange
    ( 31, 119, 180),   # 10 small-vehicle      — dark blue
    (220,  20,  60),   # 11 helicopter         — crimson
    (127, 127, 127),   # 12 roundabout         — gray
    (  0, 128, 255),   # 13 soccer-ball-field  — sky blue
    ( 60, 179, 113),   # 14 swimming-pool      — sea green
    (255,  20, 147),   # 15 container-crane    — deep pink
]
...
def plot_class_distribution(class_counts: dict[str, int], save_path: Path) -> None:
    ...
    ordered = [(DOTA_NAMES[i], class_counts.get(DOTA_NAMES[i], 0))
               for i in range(len(DOTA_NAMES))]
    names_ordered  = [x[0] for x in ordered]
    counts_ordered = [x[1] for x in ordered]
    colors_rgb     = [(c[2] / 255, c[1] / 255, c[0] / 255) for c in _COLORS_BGR]
    ...
    ax.set_title("DA-YOLO Detection Frequency — DOTA 1.5 Val Images",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="DA-YOLO inference on DOTA 1.5 aerial tiles.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--weights",     default="runs/da_yolo/dota_scratch2/weights/best.pth")
    p.add_argument("--source",      default="/data/DOTA/yolo/images/val",
                   help="Image directory or single image file")
    p.add_argument("--output",      default="infer_output/dota",
...
def main() -> None:
    ...
    print("=" * 72)
    print("  DA-YOLO — DOTA 1.5 Inference Pipeline")
    print("=" * 72)
    print(f"  Source     : {source}  ({len(image_paths)} images)")
    print(f"  Weights    : {REPO_ROOT / args.weights}")
    print(f"  Output     : {out_root}")
    print(f"  Img size   : {args.imgsz}px  (DOTA 1.5 pre-tiled resolution)")
