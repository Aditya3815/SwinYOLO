#!/usr/bin/env python3
"""
evaluate_dior.py — DA-YOLO Comprehensive Evaluation on DIOR-R
=============================================================
Computes paper-ready metrics on the DIOR-R validation split (5863 images, 20 classes):
  • mAP@0.5 and mAP@0.5:0.95 (COCO-style)
  • Per-class AP@0.5 (all 20 DIOR-R categories)
...
# DIOR-R 20 classes (in class-ID order)
DIOR_CLASSES = [
...
def write_text_table(metrics: dict, ckpt_info: dict, model_info: dict,
    ...
    lines = [
        "=" * w,
        "  DA-YOLO — DIOR-R Evaluation",
        "=" * w,
        f"  Dataset            : DIOR-R val split (20 classes, 800×800)",
...
def write_latex_table(metrics: dict, save_dir: Path) -> None:
    ...
    latex = rf"""% DA-YOLO DIOR-R Results — auto-generated by evaluate_dior.py
\begin{{table}}[t]
\centering
...
def plot_per_class_ap(metrics: dict, save_dir: Path) -> None:
    ...
    ax.set_title("DA-YOLO — Per-Class AP on DIOR-R Val (20 classes)", fontsize=13, fontweight="bold")
    ax.set_xticks(x)
...
def plot_training_curves(results_csv: Path, save_dir: Path) -> None:
    ...
    fig.suptitle("DA-YOLO Training on DIOR-R", fontsize=14, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="DA-YOLO comprehensive evaluation on DIOR-R.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--weights",  default="runs/da_yolo/dior_scratch3/weights/best.pth")
    p.add_argument("--data",     default="data/dior.yaml")
    ...
    p.add_argument("--save-dir", default="eval_results/dior_v1")
    p.add_argument("--results-csv",
                   default="runs/da_yolo/dior_scratch3/results.csv")
    return p.parse_args()
...
def main() -> None:
    ...
    # 3 — Dataloader
    print(colorstr("bold", "\n[3/5] Building dataloader (DIOR-R val, 5863 images)..."))
    ...
    for fname, desc in [
        ...
        ("per_class_ap.txt",     "Per-class AP (20 classes)"),
        ("PR_curve.png",         "Precision-Recall curve"),
        ("F1_curve.png",         "F1-confidence curve"),
        ("per_class_ap_bar.png", "Per-class AP bar chart"),
