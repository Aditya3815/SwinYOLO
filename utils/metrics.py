...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, WIoU=False, eps=1e-7):
    ...
    WIoU (Wise-IoU, AAAI 2023): geometric factor g = exp(ρ²/c²) down-weights easy
    centre-aligned anchors and focuses gradient on geometrically harder samples.
    Returns value r s.t. (1 - r) = WIoU loss, preserving the same call site as CIoU.
    ...
    if CIoU or DIoU or GIoU or WIoU:
        ...
        if CIoU or DIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if WIoU:
                # WIoU v1 geometric factor (AAAI 2023, arxiv:2301.10051).
                # g = exp(ρ²/c²) — grows when centres are far relative to the enclosing box,
                # assigning larger loss to geometrically misaligned predictions while
                # down-weighting easy near-centre anchors.
                # Return 1 - g*(1-iou) so that (1 - return) = WIoU loss, keeping the
                # same `lbox += (1 - iou).mean()` call site in loss.py unchanged.
                g = torch.exp(rho2 / c2)
                return 1 - g * (1 - iou)  # WIoU v1
