...
def train(hyp, opt, device, callbacks):
    ...
    for k, v in model.named_parameters():
        ...
        # NaN-to-zero gradient hook: essential for LayerNorm-ablated C3SWT blocks
        # (the paper removes internal LN from Swin blocks; without this, a single NaN
        # gradient will permanently corrupt Adam's momentum buffers)
        v.register_hook(lambda x: torch.nan_to_num(x, nan=0.0, posinf=1e4, neginf=-1e4))
    ...
    # Scheduler — supports cosine (default), cyclic, or linear
    scheduler_name = getattr(opt, 'scheduler', 'cosine')
    if scheduler_name == 'cyclic':
        # CyclicLR: oscillates between base_lr→max_lr every step_size_up*2 steps
        # Great for escaping local minima; effective at fine-tuning LR (1e-6)
        # step_size_up=500 ≈ 1 epoch on SLD dataset (batch=4, 50k images → ~12500 steps)
        scheduler = lr_scheduler.CyclicLR(
            optimizer,
            base_lr=hyp["lr0"] * 0.1,          # 1e-7 — lower bound
            max_lr=hyp["lr0"],                  # 1e-6 — upper bound (mentor's ask)
            step_size_up=500,                   # half cycle length in batches
            mode='triangular2',                 # amplitude halves each cycle
            cycle_momentum=False,               # AdamW doesn't use SGD momentum
        )
        lf = None  # unused when CyclicLR is active
        LOGGER.info(f"Using CyclicLR scheduler: base_lr={hyp['lr0']*0.1:.2e}, max_lr={hyp['lr0']:.2e}")
    elif opt.cos_lr or scheduler_name == 'cosine':
        lf = one_cycle(1, hyp["lrf"], epochs)  # cosine 1->hyp['lrf']
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)
        LOGGER.info("Using Cosine LR scheduler (one-cycle).")
    else:
        def lf(x):
            """Linear learning rate scheduler function."""
            return (1 - x / epochs) * (1.0 - hyp["lrf"]) + hyp["lrf"]
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)
        LOGGER.info("Using Linear LR scheduler.")
    ...
    scaler = torch.amp.GradScaler('cuda', enabled=amp)  # updated — cuda.amp deprecated
    stopper, stop = EarlyStopping(patience=opt.patience), False
    # ValLoss EarlyStopping — fires on val_loss, not fitness (mentor's recommendation)
    # Prevents false-early-stop when mAP saturates on easy synthetic datasets
    val_loss_stopper = ValLossEarlyStopping(patience=opt.patience)
    ...
    no_augs = getattr(opt, 'no_augs', 0)  # number of final epochs with augmentation disabled
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        callbacks.run("on_train_epoch_start")
        model.train()

        # ── Close-Mosaic: disable heavy augmentation for final `no_augs` epochs ──────
        # Mirrors YOLOv8's --close-mosaic N behaviour. Turning off mosaic + copy_paste
        # in the final epochs lets the model consolidate on clean, un-augmented samples.
        # Best practice: improves final mAP by ~0.5-1.5% on dense detection tasks.
        if no_augs > 0 and epoch == (epochs - no_augs):
            LOGGER.info(
                f"\n⚡ Epoch {epoch}: Closing mosaic augmentation for final {no_augs} epochs "
                f"(epochs {epoch}–{epochs-1}). Model will train on clean samples only."
            )
            hyp['mosaic'] = 0.0
            hyp['copy_paste'] = 0.0
            # Recreate train_loader without mosaic
            train_loader, dataset = create_dataloader(
                ...
                prefix=colorstr('train (no-aug): '), shuffle=True, seed=opt.seed,
            )
            nb = len(train_loader)  # update batch count
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            with torch.amp.autocast('cuda', enabled=amp):  # updated — cuda.amp deprecated
            ...
            # NaN loss guard — skip batch if loss is invalid (protects optimizer state)
            # Essential for LayerNorm-ablated C3SWT blocks which can produce NaN spikes
            if not torch.isfinite(loss):
                LOGGER.warning(f'WARNING: non-finite loss at epoch {epoch} batch {i}, skipping')
                optimizer.zero_grad()
                continue
            ...
            if ni - last_opt_step >= accumulate:
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=10.0)  # clip gradients (Swin standard)
            ...
            if RANK in {-1, 0}:
                ...
                # -- DETR-Style Custom Logger --
                if 't0' in locals():
                    eta_s = int((time.time() - t0) / (ni + 1) * ((nb * epochs) - ni - 1)) if ni > 0 else 0
                    eta_str = str(timedelta(seconds=eta_s))
                else:
                    eta_str = "0:00:00"
                    
                current_lr = optimizer.param_groups[0]['lr']
                loss_bbox, loss_giou, loss_ce = mloss[0].item(), mloss[1].item(), mloss[2].item()
                total_loss = loss_bbox + loss_giou + loss_ce
                
                detr_log = (f"Epoch: [{epoch:03d}]  [{i*opt.batch_size}/{nb*opt.batch_size}]  eta: {eta_str}  lr: {current_lr:.6f}  "
                            f"class_error: 0.00  loss: {total_loss:.4f}  loss_ce: {loss_ce:.4f}  loss_bbox: {loss_bbox:.4f}  loss_giou: {loss_giou:.4f}")
                
                if i % 50 == 0 or i == nb - 1:
                    with open(save_dir / "training_log.txt", "a") as flog:
                        flog.write(detr_log + "\n")
                        flog.flush() # Ensure logs are committed to disk
                # ------------------------------
        ...
        # CyclicLR steps per batch (done inside batch loop via on_train_batch_end)
        # LambdaLR steps per epoch
        ...
        if RANK in {-1, 0}:
            ...
            # Fitness-based early stopping (original)
            stop = stopper(epoch=epoch, fitness=fi)

            # Val-loss early stopping (mentor's recommendation) — more reliable on synthetic data
            # results: (P, R, mAP@.5, mAP@.5:.95, val_box_loss, val_obj_loss, val_cls_loss)
            val_total_loss = float(results[4]) + float(results[5]) + float(results[6])
            stop_val = val_loss_stopper(epoch=epoch, val_loss=val_total_loss)
            if stop_val and not stop:
                LOGGER.info(
                    f"[ValLoss-ES] Val loss ({val_total_loss:.5f}) stagnant for "
                    f"{opt.patience} epochs — stopping via val_loss criterion."
                )
                stop = stop_val  # val_loss ES overrides fitness ES
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                torch.save(ckpt, evals_dir / "latest.pth")
                torch.save(ckpt, w / "checkpoint.pth")
                if best_fitness == fi:
                    torch.save(ckpt, w / "best.pth")
                if epoch % 50 == 0:
                    torch.save(ckpt, evals_dir / f"{epoch:03d}.pth")
...
def parse_opt(known=False):
    ...
    # DA-YOLO extensions
    parser.add_argument(
        "--no-augs", type=int, default=0,
        help="Disable mosaic+copy_paste for the last N epochs (YOLOv8 close-mosaic trick). "
             "Recommended: 10-20%% of epochs. E.g. --no-augs 20 for a 100-epoch run.",
    )
    parser.add_argument(
        "--scheduler", type=str, default="cosine", choices=["cosine", "linear", "cyclic"],
        help="LR scheduler: cosine (one-cycle, default), linear, or cyclic (CyclicLR triangular2). "
             "Use --scheduler cyclic for fine-tuning oscillation at low LR (1e-6).",
    )
